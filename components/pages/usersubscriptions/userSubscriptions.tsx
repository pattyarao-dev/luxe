...
export const UserSubscriptions = ({userId}: {userId: string}) => {
  ...
  const handleSubscription = async (brandId: string) => {
    try {
        ...
        const data = await response.json();
        setSubscriptions(data); // Assuming the response contains the updated list of subscriptions
    } catch (error) {
                ...
                <div className="w-full flex items-center justify-between">
                  ...
                  <div className="flex flex-col gap-2 p-4 justify-between">
                    ...
                    <div className="w-full flex gap-4 text-sm">
                      ...
                      <button onClick={() => handleSubscription(String(brand._id))} className="px-3 py-0.5 border border-dark-pink rounded-lg">Unsubscribe</button>
                    </div>
                  </div>
                </div>
