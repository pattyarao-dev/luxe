...
export default function Header({ params }: { params: { slug: string } }) {
    ...
    return (
        <>
            ...
            {showSideMenu && (
                <>
                    <div className="fixed z-40 w-full min-h-screen bg-midnight-blue/70 side-menu-overlay">
                        {" "}
                    </div>
                    ...
                    <div className="fixed z-40 w-[90%] h-screen bg-white/80 drop-shadow-lg side-menu">
                        ...
                        <section className="w-full p-6 flex flex-col gap-8">
                            <Link
                                ...
                                onClick={(e) => setShowSideMenu(false)}
                            >
                                <button className="bg-midnight-blue/30 p-4 w-full text-left font-semibold uppercase rounded-xl text-white drop-shadow-sm">
                                    My Profile
                                </button>
                            </Link>
                            <Link
                                href={`/subscriptions/${params.slug}`}
                                onClick={(e) => setShowSideMenu(false)}
                            >
                                <button className="bg-midnight-blue/30 p-4 w-full text-left font-semibold uppercase rounded-xl text-white drop-shadow-sm">
                                    My Subscriptions
